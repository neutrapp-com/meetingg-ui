...
<div class="flex w-full divide-1 divide-x divide-light">
    <div class="flex flex-col divide-1 divide-y divide-light w-3/4">
        ...
        <div class="flex w-full items-center justify-evenly h-1/5">
            ...
            <div class="flex">
                <div @click="navigateTo('/')" class="border border-light border-opacity-5 h-12 bg-red-600 rounded-lg items-center justify-center flex px-4 cursor-pointer">
    ...
    <div class="flex flex-col w-1/4 divide-1 divide-y divide-light">
        ...
        <div v-if="tab.name =='participants'" class="flex p-4 flex-col w-full space-y-4 pt-4 scroll">
            <participant  class="pr-4" v-for="participant in getParticipants" v-bind:key="participant.id" :participant="participant"/>
        </div>
        <div v-if="tab.name =='viewers'" class="flex p-4 flex-col w-full space-y-4 pt-4 scroll">
            <participant  class="pr-4" v-for="viewer in getViewers" v-bind:key="viewer.id" :viewer="true" :participant="viewer"/>
        </div>
